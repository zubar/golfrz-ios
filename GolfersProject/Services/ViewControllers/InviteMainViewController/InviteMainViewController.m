    ...
    switch ([self.segmentControl selectedSegmentIndex]) {
        ...
        case 2:
            if ([invities[kSMSInvites] count] <= 0) {
                ...
                NSString * invitationMessage = [NSString stringWithFormat:@"%@", MessageToDownloadApp];
...
-(void)sendEmailToContacts:(NSArray *)mContacts{
    ...
    [controller setMessageBody:[NSString stringWithFormat:@"%@", MessageToDownloadApp] isHTML:NO];
...
- (void)appInviteDialog:(FBSDKAppInviteDialog *)appInviteDialog didCompleteWithResults:(NSDictionary *)results{
    ...
    if ([results[@"didComplete"] integerValue] ==1) {
        ...
        if ([[results allKeys] count] == 1){
            [[[UIAlertView alloc] initWithTitle:@"Invitation Sent" message:@"Sent invitations successfully" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil, nil] show];
        }
    }
    [self.segmentControl setSelectedSegmentIndex:1];
    [self setSegmentControlImagesForSelectedSegment:1];
